...
if 'ocr_counts' in st.session_state:
    ...
    st.table(pd.DataFrame([[m, c] for m, c in counts.items()], columns=["Model", "Count"]))
    ...
    with st.form("box_form"):
        ...
        if action == 'add':
            add_val = st.number_input("➕ Enter boxes to ADD:", min_value=0, step=1, key="add_val")
        elif action == 'remove':
            remove_val = st.number_input("➖ Enter boxes to REMOVE:", min_value=0, step=1, key="remove_val")
